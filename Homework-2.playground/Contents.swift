...
The `generateRandomArrayOfIntsAndNils` function is part of this playground that you cannot see on
 this page.  It creates an array that is a random mix of Int values and nils.
...
/*:
 ...
 There are two dictionaries defined elsewhere in this playground for two different fictional
 airlines, NorcalAir and SocalAir named `norcal` and `socal`, respectively, representing the
 airports and cities that the two airlines serve.
...
## 5. Sort array
 ...
 Write Swift code that will sort array4 from lowest to highest using a sort algorithm
 of your choosing.  You might want to review the logic for Selection Sort or Bubble Sort
 online.  Resist the temptation to find a sort online that is already written in Swift
 and copy and paste it.  Do not use Swift's sort method.  You should consider a nil to
 be less than any element with a value.  For example, if array4 is originally
 `[3, nil, 1, nil, 8]` it should be `[nil, nil, 1, 3, 8]` after sorting.
